...
config.texcompile = {
  latex_command = [[c:\francesco\logiciels\miktex\miktex\bin\x64\pdflatex.exe]],
  view_command = [[c:\francesco\logiciels\miktex\miktex\bin\x64\miktex-texworks.exe]],
}
...
command.add("core.docview", {
  ...
  ["texcompile:test"] = function()
    ...
    local cmd = {config.texcompile.latex_command, texname}
    -- local cmd = { [[C:\Windows\System32\ping.exe]], "ewiprd.cr2.st.com" }
    local options = {
      cwd = texpath,
    ...
    local thread_stdout = function()
      local output = ""
      while output == "" do
        output = proc:read_stdout(1024)
        print(output) -- FIXME: debug
        coroutine.yield(0.1)
      end
    ...
    local thread_stderr = function()
      local output = ""
      while output == "" do
        output = proc:read_stderr(1024)
        print("ERROR", output) -- FIXME: debug
        coroutine.yield(0.1)
      end
    end
